...
export default function App() {
  ...
  // 2. BUSCA DE DADOS SÓ ACONTECE SE HOUVER USUÁRIO
  useEffect(() => {
    if (!user) {
      setClients([]);
      return;
  ...
  const handleLogin = async () => {
    try {
      ...
      alert("Houve um erro ao tentar entrar com o Google (Normal se estiver a testar dentro do Canvas).");
    }
  };
  ...
  // O Wraper principal que ativa o modo escuro no HTML
  return (
    <div className={`${isDarkMode ? 'dark' : ''}`}>
      <div className="min-h-screen bg-slate-50 dark:bg-slate-900 pb-24 text-slate-800 dark:text-slate-100 font-sans transition-colors duration-300">
        ...
        {/* TELA DE CARREGAMENTO INICIAL MINIMALISTA */}
        {isAuthLoading ? (
          ...
          <div className="min-h-screen bg-slate-50 dark:bg-slate-900 flex flex-col items-center justify-center p-6 transition-colors duration-300">
            <div className="bg-white dark:bg-slate-800 p-8 rounded-[2.5rem] shadow-[0_8px_30px_rgb(0,0,0,0.04)] dark:shadow-none w-full max-w-sm flex flex-col items-center text-center border border-slate-100 dark:border-slate-700 transition-colors duration-300">
            ...
            {/* BOTÃO DE MODO NOTURNO NA TELA DE LOGIN */}
            <button 
              onClick={() => setIsDarkMode(!isDarkMode)} 
              className="mt-8 p-3 rounded-full bg-white dark:bg-slate-800 text-slate-400 dark:text-slate-300 shadow-sm border border-slate-100 dark:border-slate-700 transition"
            >
